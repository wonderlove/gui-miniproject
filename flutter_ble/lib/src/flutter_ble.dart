...
class FlutterBle {
  ...
  static FlutterBle _instance = new FlutterBle._();
  static FlutterBle get instance => _instance;
  ...
  ///
  Stream<ScanResult> scanForGlucometer({
    ScanMode scanMode = ScanMode.lowLatency,
    List<Guid> withDevices = const [],
    Duration timeout
  }) async* {
    var withServices = [ new Guid(GLUCOSE_SERVICE_UUID)];
    var settings = protos.ScanSettings.create()
      ..androidScanMode = scanMode.value
      ..serviceUuids.addAll(withServices.map((g) => g.toString()).toList());
    StreamSubscription subscription;
  ...
  /// Scanning for glucometer device services
  ///
  Stream<ScanResult> scanForBPM({
    ScanMode scanMode = ScanMode.lowLatency,
    List<Guid> withDevices = const [],
    Duration timeout
  }) async* {
    var withServices = [ new Guid(BPM_SERVICE_UUID)];
    var settings = protos.ScanSettings.create()
      ..androidScanMode = scanMode.value
      ..serviceUuids.addAll(withServices.map((g) => g.toString()).toList());
    StreamSubscription subscription;
    ...
    yield* controller.stream
        .map((buffer) => new protos.ScanResult.fromBuffer(buffer))
        .map((p) => new ScanResult.fromProto(p))
        .where((scanResult) => BleHelpers.maybeParseBPM(scanResult));
  }

  /// Todo refactor to search in a better way, need to improve dart skills
  Stream<ScanResult> scanForScale({
    ScanMode scanMode = ScanMode.lowLatency,
    List<Guid> withDevices = const [],
    Duration timeout
  }) async* {
    ...
    yield* controller.stream
        .map((buffer) => new protos.ScanResult.fromBuffer(buffer))
        .map((p) => new ScanResult.fromProto(p))
        .where((scanResult) => BleHelpers.maybeScaleProtocolV1(scanResult));
  }
  ...
  /// Sets the log level of the FlutterBle instance
