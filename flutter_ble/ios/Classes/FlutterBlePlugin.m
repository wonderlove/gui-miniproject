...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
  if ([@"setLogLevel" isEqualToString:call.method]) {
    ...
    @try {
      ...
      [_centralManager connectPeripheral:[self device] options:nil];
      //result(nil);
    ...
    @try {
      CBPeripheral *peripheral = [self findPeripheral:remoteId];
        self.device = peripheral;
      [_centralManager cancelPeripheralConnection:[self device]];
      result(nil);
    } @catch(FlutterError *e) {
      result(e);
    }
...
- (CBPeripheral*)findPeripheral:(NSString*)remoteId {
  ...
  CBPeripheral *peripheral;
    ...
    if([[[self device].identifier UUIDString] isEqualToString:remoteId]) {
        peripheral = self.device;
        printf("--------------------------------> arrived, i got the references");
    } else {
        printf("--------------------------------> error, i got the references");
